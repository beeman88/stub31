...
curl -v -u%3:%4 http://localhost:%1/sdata/billingboss/crmErp/-/%5?%select%
...
curl -v -u%3:%4 -X POST -d "<entry><id/><title/><updated/><payload><digest/></payload></entry>" -H "Content-Type: application/atom+xml; charset=utf-8" "http://localhost:%1/sdata/billingboss/crmErp/-/%5/$syncSource?trackingID=%2&runName=%5&runStamp=2010-10-14T08:51:02&%syncSelect%"
...
REM create sync request
curl -v -u%3:%4 -X POST -d "<entry><id/><title/><updated/><payload><digest/></payload></entry>" -H "Content-Type: application/atom+xml; charset=utf-8" "http://localhost:%1/sdata/billingboss/crmErp/-/%5/$syncSource?trackingID=%2&runName=customers&runStamp=2010-10-14T08:51:02&%syncSelect%"
...
REM get all customers
curl -v http://localhost:%1/sdata/billingboss/crmErp/-/%5?%select%
